...
// ListenUDP 类似 net.ListenUDP，但返回带有 IP_TRANSPARENT 选项的连接。
...
// ReadFromUDP 从 c 读取一个 UDP 数据包，将载荷复制到 b 中。
// 返回复制到 b 的字节数以及数据包上的返回地址。
//
// 同时读取带外数据，以便识别和解析原始目标地址。
...
// ListenPacket 类似 net.ListenPacket，但地址可以是非本地地址。
